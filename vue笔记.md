...
+ git命令行操作
  ...
  + **本地库操作**
    ...
    + 分支管理
      ...
      + 分支操作
        ...
        + 查看本地库自己创建的分支：`git branch -v`
  
        + 查看使用`fetch`，远程库自己帮我们在本地创建的分支，这个分支是从远程库的分支拉取并复制下来的，作用就是可以先看下拉取下来的代码是什么样子的，最后再决定合并不合并到自己的本地库分支。`git branch -r`
        ...
        + 合并分支
          ...
          2. 合并某个分支到当前的分支：`git merge 分支名`
             ...
             > **合并分支就是合并修改的部分，下面的每一块都是修改的部分**
             >
             > 合并分支图解一：
             >
             > ![image-20220301180556781](img/image-20220301180556781.png)
             >
             > 合并两个分支的修改部分，如果修改部分就只保留一份。
             >
             > 合并分支图解二：
             >
             > ![image-20220301182818576](img/image-20220301182818576.png)
             >
             > 如果合并时两个分支都有修改的部分，那么如果修改的是同一个地方，则会产生冲突，需要解决。如果修改的部分是不同的地方，则会自动合并代码。
  ...
  + **远程库操作**
    ...
    + 把本地库推到远程库。
      ...
      + 删除远程库地址别名：`git remote rm origin`。
      ...
      + 从远程库克隆到本地库：`git clone 远程库的地址`（默认克隆主分支）或`git clone -b 指定分支名 远程库的地址`（克隆指定分支）
  
        1. 只需要在本地创建一个文件夹，并执行克隆操作。
        2. 就会把远程库的分支里的默认分支（一般是master）所有内容都复制一份到本地库，也会叫master分支（默认分支叫什么克隆下来的分支就叫什么）。也可以指定分支。只会克隆一条分支。
        3. 会自动在该文件夹初始化一个本地库。
      ...
      + 从远程库拉取到本地库（==拉取和是在本地分支合并远程分支的操作，它的过程和结果就是合并分支==）
        ...
        > `fetch`+`merge`=`pull`
        >
        > 从远程库拉取到本地库都是只读的操作，不需要登录github的账号。但凡是写远程库，如push，就要登录账号并需要权限。
  
        > 查看本地分支：git branch -v
        >
        > 查看远程库分支：git branch -r
        >
        > 查看所有分支（远程和本地）：git branch -a
  
        拉取分步的做法：
  
        1. `git fetch origin master`：就是把远程库的`master`分支取来，并且放到本地库的`origin/master`分支。注意：`origin/master`分支是远程库的分支，所以不能通过`git branch -v`查看，要通过`git branch -r`查看远程分支。当我们切换到`origin/master`分支可以在工作区查看远程库拿来的代码，看看是否符合我们的要求，可以的话就可以选择合并，不可以就不合并。
        2. `git merge origin/master`：如果远程库的分支拿来满意的话，就可以先切换会想要合并到的分支，然后合并即可。
  
        拉取一步到位的做法：
  
        + `git pull origin master`：是上面两步的综合写法。
  
      + 协同开发时冲突的原因和解决
  
        + 本质和原因：从本地库的某个分支推送代码到远程库的某个分支，相当于分支的合并操作。把本地库分支推送到远程库分支，就是在远程库做合并分支的本质。但是由于远程库的分支如果出现冲突是无法人工解决的，于是合并操作git就强制在本地做合并，因为我们可以解决冲突。当克隆操作完成后，本地库的分支和远程库的分支是一样的，会有三种情况发生：1. 本地库做了修改，产生新的修改提交到本地库，进而产生新版本。因为远程库无法做合并操作，刚好本地库的分支就是远程库的分支，因此，推送就是把当前本地库的分支完全覆盖替换远程分支；2. 远程库做了修改，拉取下来，会在本地做合并分支操作，分支合并后的结果就是远程库的分支；3. 如果本地库分支和远程库分支都做了修改，各自产生各自的修改部分：如果各自修改的部分都是同一个文件同一个地方，产生冲突，手动解决；如果各自修改部分的位置不同，则自动合并。而第三种情况的产生既可以推送也可以拉取产生，但是推送等于在远程库做合并，无法手动解决冲突，因此，必须先拉取到本地合并，然后在推上去，因为==推送就是把当前本地库的分支完全覆盖替换远程分支。但是结果就是在远程库合并本地库分支的操作，只是实现过程不是在远程库合并本地库分支的操作而已==。
        + 结论：**推送的本质是在远程的分支合并本地的分支，但是过程比较特殊，推送的过程是把本地的分支完全替换掉远程的分支；拉取的本质和过程，都是在本地分支合并远程的分支。**
        + 解决：保持同步（克隆之后，或者推送之后）之后，有三种情况：1. 如果远程分支没有新的修改，本地库分支做了修改，其合并分支的结果就是本地库分支的代码就是远程库分支的代码，所以可以推送；2. 如果远程分支修改了，本地库分支没有修改，那么合并的结果就是远程分支的代码就是本地库的代码；3. 如果远程分支和本地分支都做了修改，那么合并分支会有冲突的可能，又因为远程无法解决冲突，也不能合并分支，只能到本地合并并解决冲突，最后推送。所以，==要先拉取，然后解决冲突（或者没有冲突），最后推送。==
  
    + 跨团队协作
  
      > 背景：如果某个人不是你们公司的成员，没有加入团队，对远程库的分支没有修改的权限。
  
      + fork操作
  
        + 原理：就是把远程库复制出一份成自己的，那么就有了对这个库的写的权限。等操作完成再把复制过来的远程库pull request发送到被你拷贝的远程库那里，让那个库的主人去审核你的代码。
  
        + 步骤：
  
          1. ![image-20220301195118764](img/image-20220301195118764.png)
  
          2. ![image-20220301195947438](img/image-20220301195947438.png)
  
          3. ![image-20220301200122520](img/image-20220301200122520.png)
  
          4. ![image-20220301200435410](img/image-20220301200435410.png)
  
          5. fork完成之后，就有了这个仓库的修改权利，因为已经变成是你的了。可以克隆到本地去做修改。
  
          6. 本地修改后，推送到远程仓库的分支，点击pull request。
  
             ![image-20220301220041271](img/image-20220301220041271.png)
  
          7. ![image-20220301220143446](img/image-20220301220143446.png)
  
          8. ![image-20220301220313140](img/image-20220301220313140.png)
  
          9. ![image-20220301220516431](img/image-20220301220516431.png)
  
          10. 你把pull request操作完成之后，就没事了。
  
          11. 接下来就是被fork的仓库的主人去审核你的代码。
  
              ![image-20220301221213179](img/image-20220301221213179.png)
  
          12. ![image-20220301221348115](img/image-20220301221348115.png)
  
          13. ![image-20220301221914828](img/image-20220301221914828.png)
  
          14. ![image-20220301222450931](img/image-20220301222450931.png)
  
          15. 最后可以拉取到本地运行。
  
  + Git在工作的使用方式（==重要==）
    
    1. 如果远程库只有一条master分支，且经理让你不要直接在master上改动：
       + 你可以先`git clone 远程库地址`，默认克隆master分支到你的本地库。
       + 然后可以在本地库新建一条比如develop分支`git branch develop`，在这条分支改动。改完了之后，把这条develop分支推送上去并在远程库新建develop分支：`git push origin develop:develop`=>简写就是：`git push origin develop`。
       + 这样子就不会污染master分支。
    2. 如果远程库有多条分支，经理只让你在hot_fix分支修改：
       + 因为默认克隆下来的是master分支，而你要在hot_fix分支修改，所以只能指定克隆的分支。用`git clone -b 指定分支名  远程库地址`。
       + 修改完了再推送上去那条分支，`git push origin hot_fix`。
    3. 如果要把远程的所有分支都克隆到本地库，以在本地合并所有的分支：
       + 先克隆主分支，`git clone 远程地址名`。
       + 然后查看远程库的所有分支：`git branch -r`
       + 然后在本地库新建分支并切换到该分支，新建的分支是从远程库克隆来的：`git checkout -b 新命名的分支名 要克隆的远程库分支名`，如`git checkout -b dev origin/dev`，就是把origin/dev的分支克隆到本地并叫做dev，且切换到dev分支。
       + 那么以后就可以在这个分支做拉取和推送。
